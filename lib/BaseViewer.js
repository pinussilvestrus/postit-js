 ...
 */
BaseViewer.prototype.importXML = function(xml, rootBoard, done) {

  if (isFunction(rootBoard)) {
    done = rootBoard;
    rootBoard = null;
  }
  ...
  this._moddle.fromXML(xml, 'postit:Definitions', function(err, definitions, context) {
    ...
    self.importDefinitions(definitions, rootBoard, function(err, importWarnings) {
 ...
 * @param {Function} [done] invoked with (err, warnings=[])
 */
BaseViewer.prototype.importDefinitions = function(definitions, rootBoard, done) {
  ...
  return this.open(rootBoard, done);
};
 ...
 * @param {Function} [done] invoked with (err, warnings=[])
 */
BaseViewer.prototype.open = function(rootBoardOrId, done) {

  if (isFunction(rootBoardOrId)) {
    done = rootBoardOrId;
    rootBoardOrId = null;
  }

  var definitions = this._definitions;
  var rootBoard = rootBoardOrId;
  ...
  if (typeof rootBoardOrId === 'string') {
    rootBoard = findRootBoard(definitions, rootBoardOrId);

    if (!rootBoard) {
      return done(new Error('PostitRootBoard <' + rootBoardOrId + '> not found'));
    }
  }
  ...
  return importPostitDiagram(this, definitions, rootBoard, done);
};
