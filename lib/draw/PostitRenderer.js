  ...
  this.handlers = {
    ...
    'postit:Image': function(parentGfx, element) {
      ...
      if (!imageSource) {
        ...
        // stacked image top image
        const source2 = bo.get('source2') || element.source2;
        if (source2) {
          let gfx_stack;

          // stacked image top text
          if (isSourceText(source2)) {

            const offsetBox = { width: 400, height: 300, x: 50, y: 50 };
            renderLabel(parentGfx, source2, {
              box: offsetBox,
              fitBox: false,
              align: 'right',
              style: {
