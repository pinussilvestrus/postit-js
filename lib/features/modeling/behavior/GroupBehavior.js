...
/**
 * Postit specific Group behavior
 */
export default function GroupBehavior(
  ...
  this.execute('shape.create', function(event) {
    ...
    if (is(businessObject, 'postit:Group') && !businessObject.categoryValueRef) {

      var definitions = getDefinitions(),
          categoryValue = createCategoryValue(definitions, postitFactory);
  ...
  eventBus.on('moddleCopy.canCopyProperty', HIGH_PRIORITY, function(context) {
    ...
    if (is(property, 'bpmn:CategoryValue')) {
      categoryValue = createCategoryValue(getDefinitions(), postitFactory);
